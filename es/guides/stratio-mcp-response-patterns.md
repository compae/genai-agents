...
2. **Delega la inspección del fichero al subagente del runtime** (p. ej. `explore` de OpenCode vía la tool Task) para preservar tu propio contexto. El subagente NO ve la conversación padre — solo ve el prompt que tú escribes. El prompt DEBE incluir:
   ...
   - **Si el fichero guardado es un payload JSON minificado en una sola línea** — la forma habitual de los outputs de los MCPs de Stratio — dile explícitamente al subagente que las herramientas line-based no le servirán y pueden **fallar de forma dura**: el `Grep` del runtime envuelve ripgrep `rg --json`, que aborta con `Ripgrep JSON record exceeded 65536 bytes` ante cualquier línea de más de ~64 KB (justo la forma de un payload minificado), y `Read` trunca cada línea a un tope fijo de caracteres no configurable en OpenCode. **Si te topas con ese error de ripgrep, NO reintentes `Grep` con otros patrones — pasa directamente a un parser estructural.** Indícale que use `jq` para consultas one-shot (p. ej. `jq '.tables | length'`, `jq '.tables[:20] | .[].name'`), o un script Python corto para extracciones más ricas (el sandbox lleva Python con la librería estándar `json` — el subagente puede ejecutar un `python -c '...'` inline o guardar un script breve).
...
3. **Si el subagente devuelve "file not found" o "permission denied"** sobre la ruta guardada, NO reintentes y NO vuelvas a delegar. Procesa el fichero tú mismo en esta conversación. Para un payload JSON de una sola línea, usa un parser estructural vía Bash (`jq`, o un `python -c '...'` inline) — las mismas primitivas que le habrías dado al subagente — porque `Grep` falla de forma dura ante la línea sobredimensionada (ver arriba). Solo para contenido que no sea JSON recurre a `Grep` para patrones concretos y `Read` con `offset` y `limit` pequeño (≤ 200 líneas por llamada). Nunca leas de extremo a extremo en una sola llamada.
