...
2. **Delegate the file inspection to the runtime's subagent** (e.g. OpenCode's `explore` via the Task tool) to preserve your own context. The subagent has NO view of the parent conversation — it only sees the prompt you write. The prompt MUST contain:
   ...
   - **If the saved file is a JSON payload minified on a single line** — the usual shape for Stratio MCP outputs — say so explicitly to the subagent and tell it that line-based tools won't help and can **hard-fail**: the runtime's `Grep` wraps ripgrep `rg --json`, which aborts with `Ripgrep JSON record exceeded 65536 bytes` on any single line over ~64 KB (exactly the shape of a minified payload), and `Read` truncates each line to a fixed character cap that is not configurable in OpenCode. **If you hit that ripgrep error, do NOT retry `Grep` with other patterns — switch straight to a structural parser.** Direct the subagent to use `jq` for one-shot queries (e.g. `jq '.tables | length'`, `jq '.tables[:20] | .[].name'`), or a short Python script for richer extraction (the sandbox ships Python with `json` in the standard library — the subagent can run an inline `python -c '...'` or save a small script).
...
3. **If the subagent returns "file not found" or "permission denied"** on the saved path, do NOT retry and do NOT delegate again. Process the file yourself in this conversation. For a single-line JSON payload, use a structural parser via Bash (`jq`, or an inline `python -c '...'`) — the same primitives you would have given the subagent — because `Grep` hard-fails on the oversized line (see above). Only for non-JSON content fall back to `Grep` for targeted patterns and `Read` with `offset` and a small `limit` (≤ 200 lines per call). Never read end-to-end in a single call.
